...
export default function Config({user,onAtualizar}){
  ...
  return(<div style={{padding:"0 4px"}}>
    ...
    {msg&&<div style={{padding:"11px 14px",borderRadius:12,marginBottom:14,fontSize:13,background:msg.t==="success"?"rgba(217,119,6,.1)":"rgba(239,68,68,.1)",color:msg.t==="success"?"#fbbf24":"#ef4444",border:"1px solid "+(msg.t==="success"?"rgba(217,119,6,.25)":"rgba(239,68,68,.2)")}}>{msg.m}</div>}

    <div style={{background:"#1a110a",borderRadius:18,padding:"18px",marginBottom:12,border:"1px solid rgba(255,255,255,0.06)"}}>
      <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16}}>
        <div style={{width:52,height:52,borderRadius:16,background:"linear-gradient(135deg,#d97706,#92400e)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,fontWeight:800,color:"#fff"}}>
        ...
        <div>
          ...
          <div style={{color:"#f59e0b",fontSize:11,marginTop:2,fontWeight:600}}>{user.role==="admin"?"Administrador":"Usuário"}</div>
        </div>
      </div>
    ...
    <div style={{background:"#1a110a",borderRadius:18,padding:"18px",border:"1px solid rgba(255,255,255,0.06)"}}>
      ...
      <div style={{color:"#475569",fontSize:13,lineHeight:1.8}}>
        <div>🏗️ ObrasControle v1.1</div>
        <div>Gestão de obras e construção civil</div>
        <div style={{marginTop:8,color:"#334155"}}>Funcionalidades: obras, etapas, gastos, pagamentos, diário de obra, relatórios PDF</div>
      </div>
    </div>
      ...
      {importInfo&&<div>
        ...
        <div style={{background:"rgba(255,255,255,0.04)",borderRadius:12,padding:"14px",marginBottom:16}}>
          <div style={{color:"#94a3b8",fontSize:13,lineHeight:2}}>
            ...
            🏗️ Obras: <strong style={{color:"#f1f5f9"}}>{importInfo.obras}</strong><br/>
            💰 Gastos: <strong style={{color:"#f1f5f9"}}>{importInfo.gastos}</strong><br/>
            📋 Etapas: <strong style={{color:"#f1f5f9"}}>{importInfo.etapas}</strong><br/>
            💳 Pagamentos: <strong style={{color:"#f1f5f9"}}>{importInfo.pagamentos}</strong><br/>
            📓 Diário: <strong style={{color:"#f1f5f9"}}>{importInfo.diario}</strong>
          </div>
        </div>
        <div style={{display:"flex",gap:8}}>
