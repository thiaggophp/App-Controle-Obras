...
export default function Admin({currentUser}){
  ...
  useEffect(()=>{
    load();
    pb.collection("obras_signup_requests").subscribe("*",()=>load());
    pb.collection("obras_accounts").subscribe("*",()=>load());
    return()=>{pb.collection("obras_signup_requests").unsubscribe("*");pb.collection("obras_accounts").unsubscribe("*")};
  },[]);
  ...
  return(<div style={{padding:"0 4px"}}>
    ...
    <div style={{display:"flex",gap:6,marginBottom:16,overflowX:"auto",paddingBottom:4}}>
      {navTabs.map(t=><button key={t.id} onClick={()=>{setTab(t.id);setMsg(null)}}
        style={{padding:"8px 14px",borderRadius:12,border:"none",background:tab===t.id?O:"rgba(255,255,255,0.05)",color:tab===t.id?"#fff":"#94a3b8",fontSize:13,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:4}}>
        {t.icon} {t.label}
        {t.badge>0&&<span style={{background:"#ef4444",color:"#fff",borderRadius:10,padding:"1px 6px",fontSize:10,fontWeight:700}}>{t.badge}</span>}
      </button>)}
    </div>

    {msg&&<div style={{padding:"11px 14px",borderRadius:12,marginBottom:12,fontSize:13,background:msg.t==="success"?"rgba(217,119,6,.1)":"rgba(239,68,68,.1)",color:msg.t==="success"?"#fbbf24":"#ef4444",border:"1px solid "+(msg.t==="success"?"rgba(217,119,6,.25)":"rgba(239,68,68,.2)")}}>{msg.m}</div>}
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{color:"#f1f5f9",fontWeight:600,fontSize:14,display:"flex",alignItems:"center",gap:6}}>
                ...
                {a.role==="admin"&&<span style={{fontSize:10,background:"rgba(217,119,6,.2)",color:"#fbbf24",padding:"2px 8px",borderRadius:6,fontWeight:700}}>Admin</span>}
                {isSelf&&<span style={{fontSize:10,background:"rgba(217,119,6,.12)",color:"#f59e0b",padding:"2px 8px",borderRadius:6,fontWeight:700}}>Você</span>}
              </div>
            ...
            <div style={{display:"flex",gap:6}}>
              <button onClick={()=>{setPassModal(a);setNewPass("");setMsg(null)}} style={{background:"rgba(217,119,6,.1)",border:"1px solid rgba(217,119,6,.25)",borderRadius:8,padding:"7px 9px",fontSize:13,color:"#fbbf24",cursor:"pointer"}}>🔑</button>
