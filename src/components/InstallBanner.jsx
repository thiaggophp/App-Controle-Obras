...
export default function InstallBanner(){
  ...
  const[gone,setGone]=useState(()=>!!localStorage.getItem("obras-pwa-ok"));
  ...
  const dismiss=()=>{localStorage.setItem("obras-pwa-ok","1");setGone(true);};
  const install=async()=>{
    if(!prompt)return;
    prompt.prompt();
          ...
          {showIOS?(
            <>
              ...
              <div style={{fontSize:11,color:"#64748b",lineHeight:1.4}}>
                Toque em <span style={{color:"#fbbf24"}}>Compartilhar ⬆</span> → <span style={{color:"#fbbf24"}}>Tela Inicial</span>
              </div>
            </>
          ):(
            <>
              <div style={{fontSize:12,fontWeight:700,color:"#f1f5f9",marginBottom:2}}>Instalar ObrasControle</div>
